...
int main(int argc, char** argv){
    ...
    request_attach->model2_name = "wood_cube_red";
    ...
    request_detach->model2_name = "wood_cube_red";
    ...
    target_poses.push_back(arm_target_pose_4);

    // green target pose
    auto arm_target_pose_5 = [&node] {
        geometry_msgs::msg::PoseStamped msg;
        msg.header.frame_id = "base_link";
        msg.header.stamp = node->now();
    ...
    target_poses.push_back(arm_target_pose_5);

    // bowl target pose
    auto arm_target_pose_6 = [&node] {
        geometry_msgs::msg::PoseStamped msg;
        msg.header.frame_id = "base_link";
        msg.header.stamp = node->now();
    ...
    target_poses.push_back(arm_target_pose_6);
    ...
    target_poses.push_back(home);


    std::string colors[] = {"wood_cube_red", "wood_cube_blue", "wood_cube_green"};
    int clr = 0;
    ...
    for(const auto& target_pose : target_poses){
        ...
        request_attach->model2_name = colors[clr];
        request_detach->model2_name = colors[clr];
